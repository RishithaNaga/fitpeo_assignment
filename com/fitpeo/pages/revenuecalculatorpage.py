...
class RevenueCalculatorPage:
    ...
    employee_slidertextfield_xpath='//input[@aria-invalid="false"]'
    employee_slider_xpath='//span[contains(@class, "MuiSlider-thumb")]'
    employee_slider_textfield_attributevalue='//span[contains(@class, "MuiSlider-thumb")]/child::input'
    text_total_recurring_reimbursement_xpath="//*[text()='Total Recurring Reimbursement for all Patients Per Month']/following-sibling::p"

    def __init__(self, driver):
        self.driver = driver

    def type_in_slider_text_field(self, number):
        slider_text_field_element = self.driver.find_element(By.XPATH,RevenueCalculatorPage.employee_slidertextfield_xpath )
    ...
    def move_slider_to(self, number):
        slider_element = self.driver.find_element(By.XPATH,RevenueCalculatorPage.employee_slider_xpath )
        action = ActionChains(self.driver)
        action.click_and_hold(slider_element).perform()

        current_value_element = self.driver.find_element(By.XPATH,RevenueCalculatorPage.employee_slider_textfield_attributevalue )
    ...
    def verify_total_recurring_reimbursement_per_month(self, expected):
        actual = self.driver.find_element(By.XPATH,RevenueCalculatorPage.text_total_recurring_reimbursement_xpath ).text
        if actual != expected:
            raise ValueMismatchException(
